...
function check_ck_editor() {
    ...
    if ($('#ckeditor_check').is(':checked')) {
        if (CKEDITOR.instances.title.getData() == '') {
            ...
            $("#error_check_editor").append("<p>News description is required</p>");
            return false;
        }
        else {
            $("#error_check_editor").empty();
...
function news_status_change(news_id) {
    ...
    alertify.confirm("Are you sure to " + active_text + " this news?", function (e) {
        if (e) {
            $.ajax({
                url: rootPath + '/administrative/route.php',
                data: 'ajax_page=ajax_home_slider&action=activate_deactivate_news_status&news_id=' + news_id + '&status=' + newStatus,
                dataType: 'text',
                type: 'post',
                success: function (text) {
                    if (text == 1) {
                        $("#success_message").empty();
                        $("#success_message").append('<div class="alert alert-success fade in"> <i class="fa fa-check-circle"></i> News status ' + active_text + 'd Successfully</div>');
...
function delete_news(obj, id, listorder) {
    alertify.confirm("Do you want to delete this news ?", function (e) {
        if (e) {
            $('#ajax_loader_content').show();
            $.ajax({
                url: rootPath + '/administrative/route.php',
                data: "ajax_page=ajax_home_slider&news_id=" + id + "&action=delete_news&listorder=" + listorder,
                dataType: 'json',
                type: 'POST',
                success: function (result) {
                    if (result['success'] == '1') {
                        showSuccessMessage('Successfully removed news', 'success_message', 6000);
                        ...
                        $('#success_message').html('<div class="alert alert-danger fade in"> <i class="fa fa-times-circle"></i> - Failed to removed news </div>');
                    }
