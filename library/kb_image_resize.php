...
class KBImageResize
{
    ...
    */
    ...
    static function imageCompression($source, $target, $imgSizeKB, $img_name)
    {
        if($imgSizeKB > 0) {
            ini_set('memory_limit', -1);
            $quality = 100 / ($imgSizeKB / 700); // 100 = image full-quality, divided by a ratio that always com. it within 1MB
            $quality = floor($quality);
            $info = getimagesize($source);

            if ($info['mime'] == 'image/jpeg') {
                $image = imagecreatefromjpeg($source);
                $isSavedComImg = imagejpeg($image, $target, $quality);
            } elseif ($info['mime'] == 'image/gif') {
                $image = imagecreatefromgif($source);
                $isSavedComImg = imagejpeg($image, $target, $quality);
            } elseif ($info['mime'] == 'image/png') {
                $image = imagecreatefrompng($source);
                //$isSavedComImg = imagepng($image, $target, 9); // quality for png[from 0 (no compression) to 9]
                $isSavedComImg = imagejpeg($image, $target, $quality);
            }
            ...
            if ($isSavedComImg) {
                ...
                $compImgSizeMB = filesize($target) / (1024 * 1024);
                $compImgSizeMB = round($compImgSizeMB, 2);

                if ($compImgSizeMB > 1.4) {

                    $actualImgSizeMB = $imgSizeKB / 1024;
                    $actualImgSizeMB = round($actualImgSizeMB, 2);

                    // write log for that file
                    require_once ABSLPATHROOT . 'library/kb_event_log_manager.php';
                    $kbEventLogManager = new KBEventLogManager();
                    $file_name = 'logs/kyc/kyc_' . date('Y-m') . '.txt';
                    $log_url = ABSLPATHROOT . $file_name;
                    $log_data = array(
                        "Image Name" => $img_name,
                        "Actual Size" => $actualImgSizeMB . ' MB',
                        "Size after compression" => $compImgSizeMB . ' MB');
                    $kbEventLogManager->generalLog($log_url, $log_data);
                    // end log
